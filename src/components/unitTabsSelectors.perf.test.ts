...
describe("unitTabsSelectors Performance", () => {
	it("benchmark selectAvailableYears and selectFallbackYear with large dataset", () => {
		...
		console.log(
			`selectAvailableYears x ${iterations} (size=${size}): ${(endAvailable - startAvailable).toFixed(2)}ms`,
		);
		...
		console.log(
			`selectFallbackYear x ${iterations} (size=${size}): ${(endFallback - startFallback).toFixed(2)}ms`,
		);
	});
});
