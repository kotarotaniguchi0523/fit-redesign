...
/**
 * Hono RPC API の最小 integration テスト。
 * out-of-process 依存（D1 / KV）は空データを返す fake を注入し、
 * 各ルートの HTTP 振る舞い（status・レスポンス形状・バリデーション 400・markdown ヘッダ）を
 * 旧 src/pages/api/* と byte 一致する形で検証する（AAA）。
 */
...
describe("answer routes", () => {
	it("GET /api/answer/status は { statuses } を返す（D1 空→空マップ）", async () => {
		const res = await api.request("/api/answer/status?userId=u1", {}, env());
		expect(res.status).toBe(200);
		expect(await res.json()).toEqual({ statuses: {} });
	});

	it("GET /api/answer/status は userId 無しで 400（旧 badRequest 形状）", async () => {
		const res = await api.request("/api/answer/status", {}, env());
	...
	it("GET /api/answer/history は { answers } を返す", async () => {
		const res = await api.request("/api/answer/history?userId=u1", {}, env());
		expect(res.status).toBe(200);
...
describe("timer routes", () => {
	it("GET /api/timer/load は { records, syncedAt } を返す", async () => {
		const res = await api.request("/api/timer/load?userId=u1", {}, env());
		expect(res.status).toBe(200);
	...
	it("DELETE /api/timer/clear は questionId 無しで 400", async () => {
		const res = await api.request("/api/timer/clear?userId=u1", { method: "DELETE" }, env());
		expect(res.status).toBe(400);
	});
});
...
describe("markdown route", () => {
	it("GET /api/markdown/ はサイト概要を text/markdown で返す", async () => {
		const res = await api.request("/api/markdown/", {}, env());
		expect(res.status).toBe(200);
	...
	it("GET /api/markdown/不正パス は 404", async () => {
		const res = await api.request("/api/markdown/not-a-unit", {}, env());
		expect(res.status).toBe(404);
	});

	it("GET /api/markdown/unit-base-conversion/2013 は単元 Markdown を返す", async () => {
		const res = await api.request("/api/markdown/unit-base-conversion/2013", {}, env());
		expect(res.status).toBe(200);
		const body = await res.text();
...
describe("middleware（hono-mw）", () => {
	...
	it("etag: markdown は ETag を付与し、If-None-Match 一致で 304", async () => {
		...
		const first = await api.request("/api/markdown/", {}, env());
