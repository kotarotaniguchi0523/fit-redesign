...
│   └── api/         # API も file ルート（_middleware.ts で /api/* 全体に logger/request-id/timing）
│       ├── _lib.ts          # 型付き createRoute(apiRoute, Bindings=D1/KV)・schemas・helper（_=非ルート）
│       ├── health.ts        # GET /api/health
│       ├── answer/{submit,status,history}.ts   # POST/GET/GET
│       ├── timer/{sync,load,clear}.ts          # POST/GET/DELETE
│       └── markdown.ts      # 基底+ワイルドカード二重経路は Hono sub-app + etag スコープ
...
├── constants.ts, global.d.ts, env.d.ts, index.css, style.css
...
- **API（file ルート + fetch）**: `app/routes/api/` に per-endpoint で分解（`export default`=GET / `export const POST`/`DELETE`）。共通基盤は `_lib.ts`（型付き `apiRoute`・schemas・helper、`_`=非ルート）、cross-cutting middleware は `_middleware.ts`（logger/request-id/timing）。`@hono/zod-validator` で 400、bodyLimit で 413、markdown は sub-app で etag→304。**クライアントは `fetch("/api/...")` を直接呼ぶ（hc/RPC 型クライアントは廃止）**
...
- **GEO/SEO**: `robots.txt`, `llms.txt`, `llms-full.txt`, `/api/markdown/`, JSON-LD, `sitemap.xml` 実装済み（base URL は `app/data/site.ts` の `SITE_URL` に集約）
...
### API は app/routes/api/ の file ルート（相対パスで定義）

API は `app/routes/api/` に file ルートとして置く。各ファイルは相対パスのハンドラを `export default`(GET)/`export const POST`/`DELETE` で出し、HonoX が `/api/*` へ自動マウントする。共通基盤（型付き `apiRoute`・schemas）は `_lib.ts`、cross-cutting middleware は `_middleware.ts`（いずれも `_` 接頭辞でルーティング除外）。基底+ワイルドカードの二重経路（markdown）は index.ts が基底にマッチしないため、Hono インスタンスを `export default` する単一ファイル（`markdown.ts`）の sub-app にする。
