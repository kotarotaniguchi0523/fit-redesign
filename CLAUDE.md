...
削除ファイルに依存する**新規追加ファイル**はgitのコンフリクト検出に引っかからない。マージ後は必ず `pnpm typecheck` でビルド確認すること。
...
- **CSP設定**: `public/_headers`（外部リソース追加時は要更新）

## URL構造（Astro SSG）

- トップ: `/`
- 単元ページ: `/unit-{slug}/{year}/`（例: `/unit-radix/2013/`）
- ガイド: `/guide/`
- 講義資料のみ: `/slide-only/`
...
`pnpm typecheck`（tsgo のみ）は `.astro` ファイルをチェックしないため、手動指示時には不十分。
