...
`pnpm lat:lint` enforces this rule by running the official `lat check` command.
...
`pnpm lat:lint` validates that all targets exist.
...
- `pnpm lat:lint` flags unreferenced specs and dangling code refs
...
Always run `pnpm lat:lint` after editing `lat.md/` files. It validates:
